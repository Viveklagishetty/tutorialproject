			...
			$role=$row3['role'];
			  ...
			  if($num2!=0) {
			  	...
			  	echo "<script> alert('are sure to delete account') </script>";
			   $sql = "DELETE from login where (email='$usernames')";
			   // echo "$sql";
			   $re12=mysqli_query($link, "$sql") or die("error33");

			   if ($role=='Tutor') {
			    $sql = "DELETE from tutor where (email='$usernames')";
			   // echo "$sql";
			   $re12=mysqli_query($link, "$sql") or die("error33");

			   }if ($role=='Student') {
			    $sql = "DELETE from student where (email='$usernames')";
			   // echo "$sql";
			   $re12=mysqli_query($link, "$sql") or die("error33");
			   }
			...
			<div class="user_card">
				...
				<div class="d-flex justify-content-center form_container">
					...
					<form action='' method='POST'>
				 	...
				 	<input  type="submit" name="submit" class="btn btn-primary btn-block btn-lg" value='Delete account'>
				   </div>
					</form>
				</div>
